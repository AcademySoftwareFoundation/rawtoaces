...
- Execute `python -m nanobind.stubgen -m rawtoaces`. This will create the file
  `rawtoaces.pyi` containing the typing information.
- Move `rawtoaces.pyi` to `./src/docs/api/python/rawtoaces.py`, note the 
  extension change. The file may require manual editing, this is because the 
  stub generated by nanobind contains the entries in the alphabetical order, 
  and trying to load it as a module may cause the use before declaration errors.
  The required changes should become clear after looking at the file diff.
